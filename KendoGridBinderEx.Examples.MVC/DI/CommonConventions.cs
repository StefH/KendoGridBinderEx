...
namespace KendoGridBinderEx.Examples.MVC.DI
{
    internal class CommonConventions
    {
            ...
            string excludeRegEx)
        {
            foreach (var interfaceType in interfaceTypes)
            {
                ...
                var matchingImplementations = implementations.Where(implementationType => !IsMatch(implementationType, excludeTypes, excludeRegEx)).ToList();
        ...
        private static bool IsMatch(Type type, IEnumerable<Type> excludeTypes, string excludeRegEx)
        {
            return IsMatch(type, excludeTypes) || IsMatch(type, excludeRegEx);
        }

        private static bool IsMatch(Type type, IEnumerable<Type> excludeTypes)
        {
